const STORAGE_KEY = "formWizard:transferencias:v1";
...
export class FormWizard {
  constructor({ locale, t, endpoints }) {
    ...
    this.el = {
      ...
      btnCrearTransferencia: document.getElementById("btnCrearTransferencia"),
      ...
      tblBody: document.querySelector("#tblDetalleTransferencia tbody"),
      ...
      vCodigo_basedestino: document.getElementById("vCodigo_basedestino"),
    };
  ...
  _bindEvents() {
    ...
    this.el.btnCrearTransferencia.addEventListener("click", () => this.crearTransferencia());
  ...
  _initDefaults() {
    ...
    if (!this.state.vTipo_documento) this.state.vTipo_documento = "TRA";
    if (!Array.isArray(this.state.vDetalleTransferencia) || this.state.vDetalleTransferencia.length === 0) {
      this.state.vDetalleTransferencia = [this._blankRow()];
    }
  }
  ...
  async _loadLookups() {
    ...
    this._fillSelect(this.el.vCodigo_basedestino, this.lookups.bases, {
      placeholder: this.t(this.locale, "selectBase"),
    });
    ...
    if (!this.state.vCodigo_basedestino && this.lookups.bases[0]) {
      this.state.vCodigo_basedestino = String(this.lookups.bases[0].id);
    }
  }

  async _loadNextDocument() {
    if (this.state.vNum_documento) return;
    const out = await this._safeJson(`${this.endpoints.nextNum}?tipo=TRA`, null);
  ...
  _renderNav() {
    ...
    this.el.btnCrearTransferencia.disabled = blocked;
  }

  _renderForm() {
    this.el.vFecha.value = this.state.vFecha || "";
    this.el.vTipo_documento.value = this.state.vTipo_documento || "TRA";
    ...
    this.el.vCodigo_basedestino.value = this.state.vCodigo_basedestino || "";
    this._renderTable();
  }
  ...
  _renderTable() {
    ...
    tbody.innerHTML = "";
    if (this.state.vDetalleTransferencia.length === 0) return;

    this.state.vDetalleTransferencia.forEach((row, idx) => {
  ...
  _renderSummary() {
    ...
    const baseOrigin = this.lookups.bases.find((b) => String(b.id) === String(this.state.vCodigo_base));
    const baseDest = this.lookups.bases.find((b) => String(b.id) === String(this.state.vCodigo_basedestino));
    const totalQty = this.state.vDetalleTransferencia.reduce((acc, row) => acc + Number(row.cantidad || 0), 0);
    const lines = this.state.vDetalleTransferencia.length;
    ...
    this.el.summary.innerHTML = `
      ...
      <div class="d-flex align-items-center justify-content-between">
        <div class="text-secondary small">${escapeHtml(this.t(this.locale, "summaryOrigin"))}</div>
        <div class="fw-semibold">${escapeHtml(baseOrigin?.name || "-")}</div>
      </div>
      <div class="d-flex align-items-center justify-content-between mt-2">
        <div class="text-secondary small">${escapeHtml(this.t(this.locale, "summaryDest"))}</div>
        <div class="fw-semibold">${escapeHtml(baseDest?.name || "-")}</div>
      </div>
      <div class="d-flex align-items-center justify-content-between mt-2">
  ...
  _onTableInput(e) {
    ...
    const row = this.state.vDetalleTransferencia[idx];
    if (!row) return;
  ...
  addRow() {
    if (!Array.isArray(this.state.vDetalleTransferencia)) this.state.vDetalleTransferencia = [];
    this.state.vDetalleTransferencia.push(this._blankRow());
    this._renderTable();
    this._renderSummary();
    this._saveDebounced();
  }
  ...
  deleteRow(idx) {
    this.state.vDetalleTransferencia.splice(idx, 1);
    this._renderTable();
    this._renderSummary();
    this._saveDebounced();
  }
  ...
  _validateHeader() {
    ...
    const baseDestOk = Boolean(this.el.vCodigo_basedestino.value);
    ok = this._setValidity(this.el.vCodigo_basedestino, baseDestOk) && ok;

    return ok;
  }

  _validateDetalle() {
    const rows = this.state.vDetalleTransferencia;
    if (!rows.length) {
  ...
  async crearTransferencia() {
    ...
    try {
      ...
      const res = await fetch(this.endpoints.crearTransferencia, {
        method: "POST",
      ...
      this._setStatus(this.t(this.locale, "transferOk"));
      this._showAlert("success", this.t(this.locale, "transferOk"));
      localStorage.removeItem(STORAGE_KEY);
      this._renderAll();
    } catch (e) {
  ...
  _buildPayload() {
    this._pullFromDom();
    const detalle = this.state.vDetalleTransferencia.map((row) => ({
    ...
    return {
      transferencia: {
        ...
        vCodigo_basedestino: Number(this.state.vCodigo_basedestino),
        vTipo_documento: "TRA",
      },
      detalle,
    };
  ...
  _pullFromDom() {
    ...
    this.state.vCodigo_basedestino = this.el.vCodigo_basedestino.value;
    const docValue = this.el.vNum_documento.value;
    if (docValue !== this.t(this.locale, "docAuto")) {
      this.state.vNum_documento = docValue;
    }
    this.state.vTipo_documento = this.el.vTipo_documento.value || "TRA";
  }
