...
class FormWizard {
  ...
  updateReview() {
    ...
    if (reviewBase) {
      const baseNombre = document.getElementById('vBaseNombre').value.trim() || '--';
      const baseCodigo = document.getElementById('vCodigo_base').value.trim() || '--';
      reviewBase.innerHTML = `
        <div>Base: <strong>${baseNombre}</strong></div>
        <div>Codigo: <strong>${baseCodigo}</strong></div>
      `;
    }
