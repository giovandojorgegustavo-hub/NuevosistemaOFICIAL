...
logLine('INFO', `Iniciando servidor CU003 usando DB: ${name}`);
...
async function handleRequest(req, res) {
  ...
  if (method === 'POST' && route === '/api/paquetes/estado') {
    ...
    const permitidos = ['robado', 'standby', 'llegado'];
    if (!permitidos.includes(estadoLower)) {
      return sendJson(res, 400, { message: 'Estado no permitido.' });
    }

    const requiereCierre = ['robado', 'llegado'].includes(estadoLower);
    ...
    try {
      ...
      if (estadoLower === 'robado') {
        const [facturas] = await queryDb(
          connection,
          'SELECT codigo_pedido, codigo_cliente, codigo_base, codigo_cliente_numrecibe, ordinal_numrecibe, codigo_cliente_puntoentrega, codigo_puntoentrega, costoenvio, monto FROM mov_contable WHERE tipo_documento = ? AND numero_documento = ?',
          ['FAC', codigo]
        );
        if (!facturas.length) {
          await connection.rollback();
          return sendJson(res, 404, { message: 'Factura no encontrada.' });
        }
        ...
        const numeroNota = nextRows[0]?.next;
        const montoNota = factura.monto ?? 0;

        await queryDb(
          connection,
          'INSERT INTO mov_contable (codigo_pedido, fecha_emision, fecha_vencimiento, fecha_valor, codigo_cliente, monto, saldo, tipo_documento, numero_documento, codigo_base, codigo_cliente_numrecibe, ordinal_numrecibe, codigo_cliente_puntoentrega, codigo_puntoentrega, costoenvio) VALUES (?, NOW(), NOW(), NOW(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [
            ...
            numeroNota,
            factura.codigo_base,
            factura.codigo_cliente_numrecibe,
            factura.ordinal_numrecibe,
            factura.codigo_cliente_puntoentrega,
            factura.codigo_puntoentrega,
            factura.costoenvio
          ]
        );
        await queryDb(connection, 'CALL actualizarsaldosclientes(?, ?, ?)', [
          factura.codigo_cliente,
          'NTC',
          montoNota
        ]);
