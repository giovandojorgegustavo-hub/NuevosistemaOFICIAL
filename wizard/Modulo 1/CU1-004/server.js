...
const LOG_PREFIX = 'CU1-004';
const PORT = Number(process.env.PORT || getUseCasePort('CU1-004'));
...
app.get('/api/paquetes-standby', async (req, res) => {
  try {
    ...
    try {
      const [result] = await runQuery(conn, 'CALL get_paquetes_por_estado(?)', ['standby']);
...
app.get('/api/viaje/:codigo', async (req, res) => {
  ...
  try {
    const conn = await dbState.pool.getConnection();
    try {
      const [result] = await runQuery(conn, 'CALL get_viaje_por_documento(?)', [codigo]);
      const rows = unwrapRows(result);
      res.json({ ok: true, viaje: rows[0] || null });
    } finally {
      conn.release();
    }
  } catch (error) {
    logError(error, 'Error al cargar viaje');
    res.status(500).json({ ok: false, message: 'ERROR' });
  }
});
...
app.post('/api/confirmar', async (req, res) => {
  ...
  try {
    ...
    await runQuery(
      ...
      [codigo, 'FAC', nextOrdinal, nuevoEstado]
    );

    const [viajeResult] = await runQuery(conn, 'CALL get_viaje_por_documento(?)', [codigo]);
    const viajeRows = unwrapRows(viajeResult);
    ...
    await conn.commit();
    res.json({ ok: true, ordinal: nextOrdinal });
  } catch (error) {
    await conn.rollback();
    logError(error, 'Error al confirmar estado');
    res.status(500).json({ ok: false, message: 'ERROR' });
  } finally {
    conn.release();
  }
