class FormWizard {
  constructor() {
    ...
    this.regex = {
      estado: /^(robado|devuelto|standby|llegado)$/i,
      codigo: /^\d+$/,
    };
  ...
  getTranslations() {
    return {
      es: {
        ...
        title: 'Gestionar paquetes en camino',
        subtitle: 'Confirma el estado de paquetes en ruta con trazabilidad completa y visibilidad del viaje.',
        ...
        step1Desc: 'Revisa los paquetes en estado en camino y selecciona uno para gestionar.',
        ...
        estadoPlaceholder: 'robado / devuelto / standby / llegado',
        ...
        loadPaquetesError: 'No se pudieron cargar los paquetes en camino.',
      ...
      en: {
        ...
        title: 'Manage in-transit packages',
        subtitle: 'Confirm package status in transit with full traceability and trip visibility.',
        ...
        step2Pill: 'Detail',
        step3Pill: 'Confirm',
        step1Title: 'Select in-transit package',
        step1Desc: 'Review packages in transit and select one to manage.',
        ...
        loadPaquetesError: 'Unable to load in-transit packages.',
  ...
  async loadPaquetes() {
    try {
      ...
      const data = await this.fetchJson('/api/paquetes?estado=en%20camino');
  ...
  renderPaquetes() {
    ...
    if (!filtered.length) {
      ...
      filtered.forEach((pkg) => {
        ...
        `;
        row.querySelector('button').addEventListener('click', () => this.selectPackage(pkg));
        this.paquetesBody.appendChild(row);
      });
    }
  ...
  async loadOrdinal() {
    if (!this.selectedPackage) return;
    try {
      const data = await this.fetchJson(`/api/paquetes/ordinal?codigo=${this.selectedPackage.codigo_paquete}`);
  ...
  async handleConfirm() {
    ...
    const requiereCierre = ['robado', 'devuelto', 'llegado'].includes(estado);
