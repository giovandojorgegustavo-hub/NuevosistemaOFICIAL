...
logLine('INFO', `Iniciando servidor CU-004 usando DB: ${name}`);
...
async function handleRequest(req, res) {
  ...
  if (method === 'POST' && route === '/api/paquetes/estado') {
    ...
    const permitidos = ['robado', 'devuelto', 'standby', 'llegado'];
    if (!permitidos.includes(estadoLower)) {
      return sendJson(res, 400, { message: 'Estado no permitido.' });
    }

    const requiereCierre = ['robado', 'devuelto', 'llegado'].includes(estadoLower);
