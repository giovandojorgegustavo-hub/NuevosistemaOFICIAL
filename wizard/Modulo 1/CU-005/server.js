...
function ensureLogFile() {
  ...
  const base = `CU-005-${datePart}-${timePart}`;
  let suffix = 1;
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (pathname.startsWith('/api/')) {
        ...
        if (req.method === 'GET' && pathname === '/api/paquetes-standby') {
          const rows = await callProcedure(pool, 'CALL get_paquetes_por_estado(?)', ['standby']);
        ...
        if (
          ...
          const allowed = new Set(['robado', 'devuelto', 'empacado', 'llegado']);
          ...
          try {
            ...
            if (['robado', 'devuelto', 'empacado', 'llegado'].includes(estado)) {
              logSql(
                "UPDATE detalleviaje SET fecha_fin = NOW() WHERE tipo_documento = 'FAC' AND numero_documento = ?",
                [codigo]
              );
              await conn.query(
                "UPDATE detalleviaje SET fecha_fin = NOW() WHERE tipo_documento = 'FAC' AND numero_documento = ?",
