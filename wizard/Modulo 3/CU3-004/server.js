...
const LOG_PREFIX = 'CU3-004';
const PORT = Number(process.env.PORT || getUseCasePort('CU3-004'));
...
function ensureLogFiles() {
  ...
  global.logStreams = [primary.stream];
  global.logFiles = [primary.filepath];
...
app.get('/api/fcc-disponibles', async (req, res) => {
  ...
  const sql = `
    SELECT
      ...
      COALESCE((
        ...
        WHERE d.tipo_documento_compra = m.tipo_documento_compra
    ...
    WHERE m.tipo_documento_compra = 'FCC'
      ...
      AND (
        ...
        OR CAST(m.codigo_provedor AS CHAR) LIKE CONCAT('%', ?, '%')
        OR CAST(m.num_documento_compra AS CHAR) LIKE CONCAT('%', ?, '%')
      )
  ...
  try {
    const [rows] = await runQuery(pool, sql, [search, search, search, search]);
    res.json({ ok: true, data: rows || [] });
  } catch (error) {
    logError(error, 'Error listando FCC disponibles');
    res.status(500).json({ ok: false, message: 'ERROR_FCC_LISTADO' });
  }
});

app.post('/api/borrar-fcc', async (req, res) => {
  const tipo = String(req.body?.tipo_documento_compra || '').trim().toUpperCase();
  const numero = parsePositiveInt(req.body?.num_documento_compra);
  const codigoProvedor = parsePositiveInt(req.body?.codigo_provedor);
  const motivo = String(req.body?.motivo || '').trim();
  const motivoSafe = motivo.replace(/[\r\n\t]+/g, ' ').slice(0, 180);

  if (tipo !== 'FCC' || !numero || !codigoProvedor) {
    return res.status(400).json({ ok: false, message: 'DATOS_INVALIDOS' });
  }

  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    await runQuery(conn, 'CALL anular_fcc_proveedor(?, ?)', [numero, codigoProvedor]);

    await conn.commit();

    logLine(
      `FCC ELIMINADA: tipo=${tipo} numero=${numero} proveedor=${codigoProvedor} motivo="${motivoSafe}"`
    );
    ...
    res.json({
      ...
      data: {
        ...
        num_documento_compra: numero,
        codigo_provedor: codigoProvedor
      }
    });
  } catch (error) {
    await conn.rollback();
    const message = String(error?.sqlMessage || error?.message || 'ERROR_BORRAR_FCC');
    if (message === 'FCC_NO_ENCONTRADA') {
      return res.status(404).json({ ok: false, message });
    }
    if (message === 'REFERENCIADA_PARTIDAS' || message === 'REFERENCIADA_GASTOS') {
      return res.status(409).json({ ok: false, message });
    }
    logError(error, 'Error borrando FCC');
    res.status(500).json({ ok: false, message: 'ERROR_BORRAR_FCC' });
  } finally {
    conn.release();
  }
});
