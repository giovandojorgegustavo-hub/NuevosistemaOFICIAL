const express = require('express');
const path = require('path');
const fs = require('fs');
const mysql = require('mysql2/promise');
const yaml = require('yaml');

const ROOT_DIR = __dirname;
const LOG_DIR = path.join(ROOT_DIR, 'logs');
const ERP_CONFIG = path.join(ROOT_DIR, '..', '..', '..', 'erp.yml');

function pad(value) {
  return String(value).padStart(2, '0');
}

function timestamp() {
  const now = new Date();
  return `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())} ${pad(now.getHours())}:${pad(
    now.getMinutes()
  )}:${pad(now.getSeconds())}`;
}

function logLine(message) {
  const line = `[${timestamp()}] ${message}`;
  console.log(line);
  if (global.logStream) {
    global.logStream.write(`${line}\n`);
  }
}

function ensureLogFile() {
  if (!fs.existsSync(LOG_DIR)) {
    fs.mkdirSync(LOG_DIR, { recursive: true });
  }
  const now = new Date();
  const base = `CU2-005-${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}-${pad(
    now.getHours()
  )}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
  let counter = 1;
  let filename;
  do {
    const suffix = String(counter).padStart(3, '0');
    filename = `${base}-${suffix}.log`;
    counter += 1;
  } while (fs.existsSync(path.join(LOG_DIR, filename)));

  global.logFileName = filename;
  global.logFilePath = path.join(LOG_DIR, filename);
  global.logStream = fs.createWriteStream(global.logFilePath, { flags: 'a' });
  logLine(`LOG FILE: ${filename}`);
}

function parseDsn(dsn) {
  const tcpMatch = dsn.match(/^mysql:\/\/([^:]+):([^@]+)@tcp\(([^:]+):(\d+)\)\/(.+)$/);
  if (tcpMatch) {
    return {
      user: decodeURIComponent(tcpMatch[1]),
      password: decodeURIComponent(tcpMatch[2]),
      host: tcpMatch[3],
      port: Number(tcpMatch[4]),
      database: tcpMatch[5]
    };
  }
  const parsed = new URL(dsn);
  return {
    user: decodeURIComponent(parsed.username),
    password: decodeURIComponent(parsed.password),
    host: parsed.hostname,
    port: Number(parsed.port || 3306),
    database: parsed.pathname.replace('/', '')
  };
}

function parseErpConfig() {
  const raw = fs.readFileSync(ERP_CONFIG, 'utf-8');
  const data = yaml.parse(raw);
  if (!data || !Array.isArray(data.connections) || !data.connections.length) {
    throw new Error('No se encontro configuracion de conexiones en erp.yml');
  }
  const connection = data.connections[0];
  if (!connection.dsn) {
    throw new Error('No se encontro DSN en erp.yml');
  }
  return {
    name: connection.name || '',
    dsn: connection.dsn
  };
}

async function initDb() {
  const config = parseErpConfig();
  logLine(`DB CONFIG: name=${config.name} dsn=${config.dsn}`);
  const dbConfig = parseDsn(config.dsn);
  const database = config.name ? config.name : dbConfig.database;
  return mysql.createPool({
    ...dbConfig,
    database,
    waitForConnections: true,
    connectionLimit: 10,
    queueLimit: 0
  });
}

function logSql(sql, params) {
  const formatted = params ? `${sql} | params=${JSON.stringify(params)}` : sql;
  logLine(`SQL: ${formatted}`);
}

function normalizeRows(resultSets) {
  if (Array.isArray(resultSets)) {
    return resultSets[0] || [];
  }
  return [];
}

async function runQuery(conn, sql, params) {
  logSql(sql, params);
  return conn.query(sql, params);
}

async function getNextNumber(conn, sql, params) {
  const [rows] = await runQuery(conn, sql, params);
  const nextRaw = rows[0]?.next ?? 1;
  const next = Number(nextRaw || 1);
  return Number.isFinite(next) && next > 0 ? next : 1;
}

const app = express();
app.use(express.json({ limit: '2mb' }));

app.use((req, res, next) => {
  logLine(`ENDPOINT: ${req.method} ${req.path}`);
  next();
});

app.use(express.static(ROOT_DIR));

app.get('/api/now', (req, res) => {
  const now = new Date();
  res.json({
    fecha: `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}`,
    hora: `${pad(now.getHours())}:${pad(now.getMinutes())}`
  });
});

app.get('/api/bases', async (req, res) => {
  const sql = 'CALL get_bases()';
  try {
    const [resultSets] = await runQuery(req.app.locals.db, sql);
    res.json(normalizeRows(resultSets));
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al cargar bases.' });
  }
});

app.get('/api/productos', async (req, res) => {
  const sql = 'CALL get_productos()';
  try {
    const [resultSets] = await runQuery(req.app.locals.db, sql);
    res.json(normalizeRows(resultSets));
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al cargar productos.' });
  }
});

app.get('/api/cuentasbancarias', async (req, res) => {
  const sql = 'CALL get_cuentasbancarias()';
  try {
    const [resultSets] = await runQuery(req.app.locals.db, sql);
    res.json(normalizeRows(resultSets));
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al cargar cuentas bancarias.' });
  }
});

app.get('/api/etiquetas-gastos', async (req, res) => {
  const sql = 'CALL get_etiquetas_gastos()';
  try {
    const [resultSets] = await runQuery(req.app.locals.db, sql);
    res.json(normalizeRows(resultSets));
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al cargar etiquetas de gastos.' });
  }
});

app.get('/api/next-numdocumentostock', async (req, res) => {
  const tipo = req.query.tipo;
  if (!tipo) {
    return res.status(400).json({ message: 'Falta tipodocumentostock.' });
  }
  const sql =
    'SELECT COALESCE(MAX(numdocumentostock), 0) + 1 AS next FROM movimiento_stock WHERE tipodocumentostock = ?';
  try {
    const next = await getNextNumber(req.app.locals.db, sql, [tipo]);
    res.json({ next });
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al calcular numero de documento.' });
  }
});

app.get('/api/next-numdocumento', async (req, res) => {
  const tipo = req.query.tipo;
  if (!tipo) {
    return res.status(400).json({ message: 'Falta tipodocumento.' });
  }
  const sql =
    'SELECT COALESCE(MAX(numdocumento), 0) + 1 AS next FROM mov_contable_gasto WHERE tipodocumento = ?';
  try {
    const next = await getNextNumber(req.app.locals.db, sql, [tipo]);
    res.json({ next });
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al calcular numero de documento.' });
  }
});

app.get('/api/logs', async (req, res) => {
  try {
    const content = fs.existsSync(global.logFilePath) ? fs.readFileSync(global.logFilePath, 'utf-8') : '';
    res.json({ filename: global.logFileName || '', content });
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'No se pudo leer el log.' });
  }
});

app.post('/api/fabricacion', async (req, res) => {
  const payload = req.body || {};
  const insumos = Array.isArray(payload.vDetalleInsumos) ? payload.vDetalleInsumos : [];
  const produccion = Array.isArray(payload.vDetalleProduccion) ? payload.vDetalleProduccion : [];
  const gastos = Array.isArray(payload.vGastosFacturas) ? payload.vGastosFacturas : [];

  if (!payload.vFecha || !payload.vCodigo_base) {
    return res.status(400).json({ message: 'Faltan datos generales de fabricacion.' });
  }
  if (!insumos.length) {
    return res.status(400).json({ message: 'Debe incluir al menos un insumo.' });
  }
  if (!produccion.length) {
    return res.status(400).json({ message: 'Debe incluir al menos un producto producido.' });
  }
  if (!gastos.length) {
    return res.status(400).json({ message: 'Debe incluir al menos un gasto.' });
  }

  const conn = await req.app.locals.db.getConnection();
  const vTipoDocumentoStockInsumo = 'FIS';
  const vTipoDocumentoStockProduccion = 'FPR';
  const vTipoDocumentoStockFabricacion = vTipoDocumentoStockProduccion;
  const vTipoDocumentoGasto = 'GAS';

  try {
    await conn.beginTransaction();
    logLine('SQL: BEGIN TRANSACTION');

    const vNumDocumentoStockInsumo = await getNextNumber(
      conn,
      'SELECT COALESCE(MAX(numdocumentostock), 0) + 1 AS next FROM movimiento_stock WHERE tipodocumentostock = ?',
      [vTipoDocumentoStockInsumo]
    );
    const vNumDocumentoStockProduccion = await getNextNumber(
      conn,
      'SELECT COALESCE(MAX(numdocumentostock), 0) + 1 AS next FROM movimiento_stock WHERE tipodocumentostock = ?',
      [vTipoDocumentoStockProduccion]
    );

    const insertMovSql =
      'INSERT INTO movimiento_stock (tipodocumentostock, numdocumentostock, fecha, codigo_base) VALUES (?, ?, ?, ?)';

    await runQuery(conn, insertMovSql, [
      vTipoDocumentoStockInsumo,
      vNumDocumentoStockInsumo,
      payload.vFecha,
      payload.vCodigo_base
    ]);

    let ordinalInsumo = await getNextNumber(
      conn,
      'SELECT COALESCE(MAX(ordinal), 0) + 1 AS next FROM detalle_movimiento_stock WHERE tipodocumentostock = ? AND numdocumentostock = ?',
      [vTipoDocumentoStockInsumo, vNumDocumentoStockInsumo]
    );

    const insertDetalleSql =
      'INSERT INTO detalle_movimiento_stock (ordinal, tipodocumentostock, numdocumentostock, codigo_producto, cantidad) VALUES (?, ?, ?, ?, ?)';

    for (const item of insumos) {
      const cantidad = Number(String(item.vcantidad_insumo || '0').replace(',', '.'));
      await runQuery(conn, insertDetalleSql, [
        ordinalInsumo,
        vTipoDocumentoStockInsumo,
        vNumDocumentoStockInsumo,
        item.vcodigo_producto_insumo,
        Number.isFinite(cantidad) ? cantidad : 0
      ]);
      ordinalInsumo += 1;
    }

    await runQuery(conn, insertMovSql, [
      vTipoDocumentoStockProduccion,
      vNumDocumentoStockProduccion,
      payload.vFecha,
      payload.vCodigo_base
    ]);

    let ordinalProduccion = await getNextNumber(
      conn,
      'SELECT COALESCE(MAX(ordinal), 0) + 1 AS next FROM detalle_movimiento_stock WHERE tipodocumentostock = ? AND numdocumentostock = ?',
      [vTipoDocumentoStockProduccion, vNumDocumentoStockProduccion]
    );

    for (const item of produccion) {
      const cantidad = Number(String(item.vcantidad_producida || '0').replace(',', '.'));
      await runQuery(conn, insertDetalleSql, [
        ordinalProduccion,
        vTipoDocumentoStockProduccion,
        vNumDocumentoStockProduccion,
        item.vcodigo_producto_producido,
        Number.isFinite(cantidad) ? cantidad : 0
      ]);
      ordinalProduccion += 1;
    }

    let nextGasto = await getNextNumber(
      conn,
      'SELECT COALESCE(MAX(numdocumento), 0) + 1 AS next FROM mov_contable_gasto WHERE tipodocumento = ?',
      [vTipoDocumentoGasto]
    );

    const insertGastoSql =
      'INSERT INTO mov_contable_gasto (tipodocumento, numdocumento, fecha, descripcion, monto, codigo_cuentabancaria) VALUES (?, ?, ?, ?, ?, ?)';
    const insertGastoDetalleSql =
      'INSERT INTO mov_contable_gasto_detalle (tipodocumento, numdocumento, ordinal, codigoetiquetagasto) VALUES (?, ?, ?, ?)';
    const insertFabricacionGastoSql =
      'INSERT INTO fabricaciongastos (tipodocumentostock, numdocumentostock, tipodocumentogasto, numdocumentogasto, ordinal) VALUES (?, ?, ?, ?, ?)';

    for (const gasto of gastos) {
      const vNumDocumentoGasto = nextGasto;
      nextGasto += 1;

      const monto = Number(String(gasto.vMontoGastoTotal || '0').replace(',', '.'));

      await runQuery(conn, insertGastoSql, [
        vTipoDocumentoGasto,
        vNumDocumentoGasto,
        payload.vFecha,
        gasto.vDescripcionGasto,
        Number.isFinite(monto) ? monto : 0,
        gasto.vCodigo_cuentabancaria
      ]);

      let ordinalGasto = await getNextNumber(
        conn,
        'SELECT COALESCE(MAX(ordinal), 0) + 1 AS next FROM mov_contable_gasto_detalle WHERE tipodocumento = ? AND numdocumento = ?',
        [vTipoDocumentoGasto, vNumDocumentoGasto]
      );

      const etiquetas = Array.isArray(gasto.vDetalleGastos) ? gasto.vDetalleGastos : [];
      for (const etiqueta of etiquetas) {
        await runQuery(conn, insertGastoDetalleSql, [vTipoDocumentoGasto, vNumDocumentoGasto, ordinalGasto, etiqueta]);
        ordinalGasto += 1;
      }

      const ordinalFabricacion = await getNextNumber(
        conn,
        'SELECT COALESCE(MAX(ordinal), 0) + 1 AS next FROM fabricaciongastos WHERE tipodocumentostock = ? AND numdocumentostock = ? AND tipodocumentogasto = ? AND numdocumentogasto = ?',
        [
          vTipoDocumentoStockFabricacion,
          vNumDocumentoStockProduccion,
          vTipoDocumentoGasto,
          vNumDocumentoGasto
        ]
      );

      await runQuery(conn, insertFabricacionGastoSql, [
        vTipoDocumentoStockFabricacion,
        vNumDocumentoStockProduccion,
        vTipoDocumentoGasto,
        vNumDocumentoGasto,
        ordinalFabricacion
      ]);

      await runQuery(conn, 'CALL aplicar_gasto_bancario(?, ?)', [vTipoDocumentoGasto, vNumDocumentoGasto]);
    }

    await runQuery(conn, 'CALL get_saldo_stock(?, ?)', [
      vTipoDocumentoStockInsumo,
      vNumDocumentoStockInsumo
    ]);
    await runQuery(conn, 'CALL get_saldo_stock(?, ?)', [
      vTipoDocumentoStockProduccion,
      vNumDocumentoStockProduccion
    ]);

    await conn.commit();
    logLine('SQL: COMMIT');

    res.json({
      message: 'Fabricacion registrada correctamente.',
      vNumDocumentoStockInsumo,
      vNumDocumentoStockProduccion
    });
  } catch (error) {
    await conn.rollback();
    logLine('SQL: ROLLBACK');
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al registrar la fabricacion.' });
  } finally {
    conn.release();
  }
});

async function start() {
  ensureLogFile();
  try {
    const db = await initDb();
    app.locals.db = db;
    const port = process.env.PORT || 3000;
    app.listen(port, () => {
      logLine(`SERVER START: http://localhost:${port}`);
    });
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    process.exit(1);
  }
}

start();
