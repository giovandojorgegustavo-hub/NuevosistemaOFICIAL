...
function ensureLogFile() {
  ...
  const now = new Date();
  const base = `CU2001-${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}-${pad(
    now.getHours()
...
app.get('/api/ultima-asistencia', async (req, res) => {
  const codigoBase = Number(req.query.codigo_base);
  const codigoUsuario = Number(req.query.codigo_usuario);
  if (!Number.isFinite(codigoBase) || !Number.isFinite(codigoUsuario)) {
    return res.status(400).json({ message: 'Parametros invalidos para ultima asistencia.' });
  }

  const sql =
    'SELECT MAX(fecha) AS ultima_asistencia FROM bitacoraBase WHERE codigo_base = ? AND codigo_usuario = ?';
  try {
    const [rows] = await runQuery(req.app.locals.db, sql, [codigoBase, codigoUsuario]);
    res.json({
      ultima_asistencia: rows?.[0]?.ultima_asistencia || null
    });
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al consultar ultima asistencia.' });
  }
});
...
app.post('/api/abrir-horario', async (req, res) => {
  ...
  const vCodigo_base = Number(payload.vCodigo_base);
  const vCodigo_usuario = Number(payload.vCodigo_usuario);

  if (!vFecha || !Number.isFinite(vCodigo_base) || !Number.isFinite(vCodigo_usuario)) {
    return res.status(400).json({ message: 'Datos incompletos para abrir horario.' });
  }

  const insertSql =
    'INSERT INTO bitacoraBase (fecha, codigo_base, codigo_usuario) VALUES (?, ?, ?)';
  try {
    await runQuery(req.app.locals.db, insertSql, [vFecha, vCodigo_base, vCodigo_usuario]);
    res.json({
      message: 'Horario abierto correctamente.',
      fecha: vFecha
    });
  } catch (error) {
    logLine(`ERROR: ${error.message}`);
    res.status(500).json({ message: 'Error al abrir horario.' });
  }
});
