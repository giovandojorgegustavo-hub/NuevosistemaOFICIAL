class FormWizard {
  constructor() {
    ...
    this.packageCodeRegex = /^[A-Za-z0-9-]{3,}$/;
  ...
  getTranslations() {
    return {
      es: {
        ...
        subtitle: 'Flujo seguro y trazable para actualizar el estado de paquetes en tránsito.',
        viewLogs: 'Ver logs SQL',
        step1Title: 'Seleccion de paquete en camino',
        step1Desc: 'Selecciona un paquete para revisar sus datos y estado.',
        thCodigo: 'Codigo',
        thFecha: 'Fecha',
        thCliente: 'Cliente',
      ...
      en: {
        ...
        subtitle: 'Secure, traceable flow to update package status in transit.',
        viewLogs: 'View SQL logs',
        step1Title: 'Select in-transit package',
        step1Desc: 'Choose a package to review its data and status.',
        thCodigo: 'Code',
  ...
  async selectPackage(code) {
    if (!this.packageCodeRegex.test(code || '')) {
    ...
    this.selectedPackageMeta = this.packages?.find((item) => {
      return (item.Vcodigo_paquete || item.codigo_paquete) === code;
    });
  ...
  renderTripInfo() {
    ...
    this.deliveryInfo.innerHTML = `
      <div><span>${this.t('labelEntrega')}</span>${this.selectedPackageMeta?.vconcatenarpuntoentrega || this.selectedPackageMeta?.concatenarpuntoentrega || ''}</div>
      <div><span>${this.t('labelRecibe')}</span>${this.selectedPackageMeta?.vconcatenarnumrecibe || this.selectedPackageMeta?.concatenarnumrecibe || ''}</div>
    `;
  }
