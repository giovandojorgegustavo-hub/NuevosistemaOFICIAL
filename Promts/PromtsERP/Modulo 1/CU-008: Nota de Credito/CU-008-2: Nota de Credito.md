...
El codigo generado debe guardarse en una sola carpeta por caso de uso: `wizard/CU-008` (siempre `wizard/CU-XXX`), sobrescribiendo su propio wizard para evitar duplicados. Priorizar codigo limpio y eficiente: mientras menos codigo, mejor, sin sacrificar claridad.
...
- Guardar los mismos logs en archivo por ejecucion dentro de `wizard/CU-008/logs/`.
- El archivo debe nombrarse `CU-008-YYYYMMDD-HHMMSS-001.log` (incrementar el sufijo si ya existe).
...
2. Confirmar y emitir Nota de Credito.
...
vTipo_documento = "NTC".
vNumero_documento = correlativo numerico (12 digitos) no editable. Se debe generar con SQL:
`SELECT LPAD(COALESCE(MAX(numero_documento), 0) + 1, 12, '0') AS next FROM mov_contable WHERE tipo_documento = 'NTC'`.
vCodigo_base = Seleccionar de la lista de Bases devuelta por el SP `get_bases()` (opcional).

Si vCodigo_base tiene valor, presentar un Grid editable llamado "vProdNotaCredito" que permita agregar, borrar y editar lineas.
...
vTotalNota = suma de vPrecio_total del grid "vProdNotaCredito".
vOrdinalDetMovCont = calcular con SQL:
`SELECT COALESCE(MAX(ordinal), 0) + 1 AS next FROM mov_contable_detalle WHERE tipo_documento = vTipo_documento AND numero_documento = vNumero_documento` (si no hay filas, usar 1).

Validaciones:
- No permitir registrar la Nota de Credito si vCodigo_base tiene valor y el grid "vProdNotaCredito" no tiene lineas.
- vCodigo_cliente requerido.
- vFecha_emision requerida.


## Paso 2  Confirmar y emitir Nota de Credito.
