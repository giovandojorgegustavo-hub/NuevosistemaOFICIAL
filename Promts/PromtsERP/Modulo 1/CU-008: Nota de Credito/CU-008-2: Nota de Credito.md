...
Guardar los mismos logs en archivo por ejecucion dentro de `wizard/Modulo 1/CU-008/logs/`.
...
- Si la nota de credito es NUEVA: calcular con SQL `SELECT COALESCE(MAX(numero_documento), 0) + 1 AS next FROM mov_contable WHERE tipo_documento = 'NTC'`.
...
- Si la nota de credito es NUEVA: asignar ordinal secuencial por linea (1,2,3...) segun el indice del grid.
